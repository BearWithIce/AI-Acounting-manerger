...
import {
  handleBuyInteraction,
  handleBuySlashCommand,
  handleBuyAdminMessage,
  registerBuyCommand
...
client.once(
  ...
  async () => {
    try {
      ...
      await registerBuyCommand(client);
    } catch (error) {
      console.error(
...
client.on(
  ...
  async message => {
    try {
      ...
      if (await handleBuyAdminMessage(message)) {
        return;
      }
        ...
        const helpEmbed =
          new EmbedBuilder()
            ...
            .addFields(
              {
                name:
                  "🛒 /buy — Create Hosting Order",

                value:
                  "`/buy user:@user ram:4GB disk:50GB cpu:2vCore location:Singapore price:1500`\n\n" +
                  "The customer receives a public order message, verifies their email by OTP, selects payment, and waits for admin confirmation."
              },

              {
                name:
                  "🎟️ !add offer — Create Discount Code",

                value:
                  "`!add offer CODE 20% 30 days` or `!add offer CODE 500 2027/7/1`"
              },

              {
                ...
                value:
                  "`!confirm AURA-ORDER-2026-000001 AURA-2026-000123`"
              },
...
client.on(
  ...
  async interaction => {
    try {
      if (await handleBuySlashCommand(interaction)) {
        return;
      }

      if (await handleBuyInteraction(interaction)) {
        return;
      }
